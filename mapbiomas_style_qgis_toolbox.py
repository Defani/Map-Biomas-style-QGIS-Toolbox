...
def _apply_symbology(layer, field='gridcode'):
    ...
    for pid in ordered:
        ...
        categories.append(QgsRendererCategory(pid, sym, c_id))
...
class _GeoJsonVectorDestination(QgsProcessingParameterVectorDestination):
    """
    Subclass kecil agar dialog "Save Vector Layer As" menyarankan
    ekstensi .geojson sebagai default, tanpa membatasi pilihan format lain
    (GeoPackage .gpkg, Shapefile .shp, dll tetap bisa dipilih manual).
    """
...
class MapBiomasRasterToVectorAlgorithm(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "2. Dissolve per kelas (gridcode)<br>"
            "3. Isi atribut berdasarkan gridcode<br>"
            "4. Simbologi warna ATBD<br>"
            "5. Legenda hanya menampilkan nama kelas (tanpa kode angka)<br>"
            "6. Format output default <b>GeoJSON</b> — bisa dipilih ke GeoPackage (.gpkg) atau Shapefile (.shp) lewat dialog simpan<br>"
            "<hr>"
    ...
    def initAlgorithm(self, config=None):
        ...
        out_param = _GeoJsonVectorDestination(
            self.OUTPUT_VECTOR,
            self.tr('Output Vektor Polygon (default GeoJSON — bisa juga GeoPackage/.gpkg atau Shapefile/.shp)'),
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        feedback.setProgressText('Langkah 1/5 — Polygonize raster ke vektor ...')
        ...
        feedback.setProgressText('Langkah 2/5 — Dissolve per kelas (gridcode) ...')
        # 'TEMPORARY_OUTPUT' untuk algoritma native bisa menghasilkan memory layer
        # (bukan file fisik), sehingga tidak bisa dibuka lagi lewat provider 'ogr'.
        # Di sini kita paksa hasil dissolve ditulis ke file GeoPackage sementara
        # yang nyata, sama seperti pendekatan pada langkah polygonize.
        dissolve_tmp_path = QgsProcessingUtils.generateTempFilename('dissolved.gpkg')
        dissolved = processing.run('native:dissolve', {
            'INPUT': poly['OUTPUT'], 'FIELD': ['gridcode'], 'OUTPUT': dissolve_tmp_path,
        }, context=context, feedback=feedback, is_child_algorithm=True)
        if feedback.isCanceled(): return {}
        feedback.setProgress(30)

        feedback.setProgressText('Langkah 3/5 — Menambahkan field atribut ...')
        tmp = QgsVectorLayer(dissolved['OUTPUT'], 'tmp', 'ogr')
        if not tmp.isValid():
            raise QgsProcessingException(self.tr('Dissolve gagal.'))
        dp = tmp.dataProvider()
        dp.addAttributes([
        ...
        feedback.setProgressText('Langkah 4/5 — Mengisi tabel atribut ...')
        flds = tmp.fields()
        ...
        feedback.setProgressText('Langkah 5/5 — Menyimpan output ...')
